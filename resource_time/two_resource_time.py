...
width = 0.73
p1 = ax0.bar(ind, Bottom, width, color=color_styles[0])
p2 = ax0.bar(ind, Center, width, bottom=Bottom, color=color_styles[1])
p3 = ax0.bar(ind, Top, width, bottom=d, color=color_styles[2])
d = np.array(Bottom) + np.array(Top) + np.array(Center)
p4 = ax0.bar(ind, Other, width, bottom=d, color=color_styles[3])

# plt.gca().yaxis.set_major_formatter(ticker.FuncFormatter(to_percent))
# ax0.set_major_formatter(ticker.FuncFormatter(to_percent))
plt.axhline(0.75, linestyle=':', color='k')
ax0.axhline(0.25, linestyle=':', color='k')
ax0.locator_params(axis='y', nbins=6)
ax0.tick_params(direction='in', labelsize=font_size)
# ax0.xticks(rotation=30)
