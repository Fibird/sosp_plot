...
color_styles = ['#93cf93', '#eb9192', '#f2bae0']
markers = ['x', 'o', '>', 'square', '*', '<']
...
datas = get_data_from_sim("../isolation2/131/r2b2_iops.log")
r2b_rts = []
for data in datas:
    r2b_rts.append(stat_resource_time(data)[0])

datas = get_data_from_sim("../isolation2/131/rwl_iops.log")
rwl_rts = []
for data in datas:
    rwl_rts.append(stat_resource_time(data)[0])

datas = get_data_from_sim("../isolation2/131/rw2_iops.log")
rw_rts = []
for data in datas:
    rw_rts.append(stat_resource_time(data)[0])

datas = get_data_from_sim("../isolation2/131/rl2_iops.log")
rl_rts = []
for data in datas:
    rl_rts.append(stat_resource_time(data)[0])

datas = get_data_from_sim("../isolation2/131/wl_iops.log")
wl_rts = []
for data in datas:
    wl_rts.append(stat_resource_time(data)[0])

datas = get_data_from_sim("../isolation2/131/r_iops.log")
r_rts = []
for data in datas:
    r_rts.append(stat_resource_time(data)[0])

datas = get_data_from_sim("../isolation2/131/w2_iops.log")
w_rts = []
for data in datas:
    w_rts.append(stat_resource_time(data)[0])

datas = get_data_from_sim("../isolation2/131/l2_iops.log")
l_rts = []
for data in datas:
...
Bottom = (target_ratio[0], r2b_rts[0] / r2b_sum, rl_rts[0] / rl_sum, rw_rts[0] / rw_sum, rwl_rts[0] / rwl_sum, wl_rts[0] / wl_sum, r_rts[0] / r_sum, w_rts[0] / w_sum, l_rts[0] / l_sum)
Center = (target_ratio[1], r2b_rts[1] / r2b_sum, rl_rts[1] / rl_sum, rw_rts[1] / rw_sum, rwl_rts[1] / rwl_sum, wl_rts[1] / wl_sum, r_rts[1] / r_sum, w_rts[1] / w_sum, l_rts[1] / l_sum)
Top = (target_ratio[2], r2b_rts[2] / r2b_sum, rl_rts[2] / rl_sum, rw_rts[2] / rw_sum, rwl_rts[2] / rwl_sum, wl_rts[2] / wl_sum, r_rts[2] / r_sum, w_rts[2] / w_sum, l_rts[2] / l_sum)
...
for a, b in zip(ind, Top):
    plt.text(a, (2 - b) / 2 - 0.03, '%.1f' % (b * 100), ha='center', va='bottom', fontsize=14)
...
plt.ylim((0, 1))
plt.legend((p1[0], p2[0], p3[0]), labels, ncol=2, loc='center', bbox_to_anchor=(0.5, 0.65))
plt.tight_layout()

plt.savefig("resource_time_131.svg")
